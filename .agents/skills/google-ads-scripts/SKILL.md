...
## ⚠️ API v23 Known Issues — Read Before Writing Any Campaign Script

These are confirmed bugs/requirements for this account (Customer `2436521562`):

| Issue | Fix |
|---|---|
| **`contains_eu_political_advertising` REQUIRED** | Set `campaign.contains_eu_political_advertising = 3` (`DOES_NOT_CONTAIN_EU_POLITICAL_ADVERTISING`) on every `CampaignOperation.create` |
...
| **`python` not found** | Use `python3` always |
...
See `MANIFESTO.md` in project root for current campaign state and resource IDs.
